...
UsersAdapter(
    ...
    inner class UserViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(user: User) {
            ...
            tvUserName.text = nombreCompleto.ifEmpty { "Sin nombre" }
            tvUserEmail.text = user.email ?: "Sin email"
            tvUserRole.text = "Rol: ${user.rol ?: "cliente"}"
            
            // Configurar color según rol
            val roleColor = when ((user.rol ?: "cliente").lowercase()) {
                "admin" -> "#E74C3C"
                "cliente" -> "#3498DB"
                else -> "#95A5A6"
            }
            ...
            itemView.setOnClickListener {
                Toast.makeText(itemView.context, "Seleccionado: $nombreCompleto", Toast.LENGTH_SHORT).show()
            }
        }
    }
}
